...
| `disconnected` | the design falls into more parts than its `pieces` count allows |
...
**Blocks are 2 inches across and 1 inch tall**, so `cube_size_m` is the 50.8 mm plan
size of a cell and the 25.4 mm layer height lives in `sim/assembly_config.json`. A cell
covers four times the area it did at 25.4 mm, which is why every example below is small.

`examples/dog.json` — 16 blocks, one layer, generated by `claude-opus-5` from the
...
`examples/smiley.json` — 10 blocks, three rows, from the description "a smiley face
with two eyes":

```
...B..B..     blue eyes, two rows back and touching nothing
.O......O     orange corners, turning up
..YYYYYY.     yellow mouth
```

Declares `pieces: 5`: the mouth is one part, the two corners and the two eyes are their
own. `examples/rocket.json` — 11 blocks, a white fuselage with a red nose and four
orange fins, re-fitted for 2 inch blocks.
