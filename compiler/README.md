...
**Blocks are one-inch cubes (25.4 mm on every axis)**. `cube_size_m`
and the configured layer height are both 0.0254 m. Per-build reach, staging,
and chassis checks remain required.
...
own. `examples/rocket.json` is the restored 26-cube one-inch design.
